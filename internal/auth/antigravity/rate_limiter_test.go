...
func TestQuotaExhaustedBackoff(t *testing.T) {
	...
	// First 429 → ~60s
	info1 := rl.ParseFromError("acc1", 429, nil, []byte("{}"), &model)
...
func TestModelIsolation(t *testing.T) {
	...
	// Lock model1
	rl.ParseFromError("acc1", 429, nil, []byte("{}"), &model1)
...
func TestRateLimitExceededAccountWide(t *testing.T) {
	rl := NewAntigravityRateLimiter()
	anyModel := "any-model"
	rl.ParseFromError("acc1", 429, nil, []byte("{}"), nil)
...
func TestRateLimiterConcurrent(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		go func() {
			...
			rl.ParseFromError("acc1", 429, nil, []byte("{}"), &model)
